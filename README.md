...
**[SideStep Wizard](https://johnbuckman.github.io/SideStepWizard/)** lets an app developer hand anyone a single file that installs their app — no Xcode and no developer account on the other end. You point it at your app's GitHub repo once, and it produces a small **"‹name› installer"** you can send to people or link from your website. **[See the SideStep Wizard site →](https://johnbuckman.github.io/SideStepWizard/)**
...
(function () {
  function mount(el){
    ...
    btn.addEventListener("click",function(ev){
      ...
      // Safari pops a blocking "address is invalid" dialog when a custom scheme has no handler
      // and you navigate the TOP window — but NOT inside a hidden iframe, where a missing handler
      // fails silently. Other browsers block iframe scheme-launches, so navigate the window there.
